...
const posts = records
  .map((r) => {
    ...
    const slug = slugify(pick(r, ["Slug"]) || title);
    const bodyHtml = pick(r, ["Post Body", "Body", "Content", "Post Content"]);
    const body = bodyHtml ? td.turndown(bodyHtml) : "";
    const excerpt = pick(r, ["Post Summary", "Summary", "Excerpt", "Meta Description"]);
    const hero = pick(r, ["Main Image", "Thumbnail image", "Hero Image", "Image"]);
    const category = pick(r, ["Category", "Collection"]);
    const publishedAtRaw = pick(r, ["Published On", "Created On", "Publish Date", "Date"]);
    const publishedAt = publishedAtRaw ? new Date(publishedAtRaw).toISOString() : null;
    const oldPath = pick(r, ["Old Path", "old_path"]) || `${OLD_BLOG_BASE}/${slug}`;
    return { title, slug, body, excerpt, hero, category, publishedAt, oldPath };
  })
  .filter(Boolean);

console.log(`Parsed ${posts.length} post(s) from ${file}.`);
...
try {
  ...
  for (const p of posts) {
    await client.query(
      `INSERT INTO posts (slug, title, excerpt, body, hero_image, category, status, published_at)
